...
void Daemon::addToStartup() {
    ...
    serviceFile << "[Unit]\n";
    serviceFile << "Description=restart_daemon\n";
    ...
    int enableStatus = system("sudo systemctl enable restart_daemon.service");
    if (enableStatus != 0) {
        std::cerr << "Error: Failed to enable restart_daemon service!" << std::endl;
        // // logError("Error: Failed to enable restart_daemon service!");
        return;
    }
