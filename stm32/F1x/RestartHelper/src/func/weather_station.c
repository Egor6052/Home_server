...
/**
 * @brief Приймає з UART2 відповідь-JSON байт за байтом через кільцевий
 *        буфер (UART2_ReadByte, наповнюється HAL_UART_RxCpltCallback в
 *        uart.c), доки не зустріне '}' або не вичерпає лічильник
 *        тайм-аутів.
 *
 *        НЕ використовує HAL_UART_Receive() (блокуючий) - huart2 постійно
 *        "зайнятий" через HAL_UART_Receive_IT(), яку озброює UART2_Init()
 *        і яку сама ж HAL_UART_RxCpltCallback перевооружає щоразу. Блокуючий
 *        виклик на такому huart з точки зору основного циклу майже завжди
 *        одразу поверне HAL_BUSY замість того, щоб реально чекати байт -
 *        тому читаємо так само, як і решта проєкту, через ReadByte-поллінг.
 *
 * @return true, якщо в буфері є хоча б один символ '}' (ймовірний кінець об'єкта)
 */
static bool uart2_read_json_response(char *out_buf, size_t out_size) {
    size_t len = 0;
    uint16_t timeouts = 0;
    uint32_t last_progress_ms = milliseconds;

    out_buf[0] = '\0';

    while (len < out_size - 1 && timeouts < WS_MAX_TIMEOUTS) {
        uint8_t byte;
        if (UART2_ReadByte(&byte)) {
            out_buf[len++] = (char)byte;
            out_buf[len] = '\0';
            timeouts = 0;
            last_progress_ms = milliseconds;
            if (byte == '}') {
                break;
            }
        } else if ((milliseconds - last_progress_ms) >= WS_BYTE_TIMEOUT_MS) {
            timeouts++;
            last_progress_ms = milliseconds;
        }
    }
...
void update_weather_station_data(void) {
    ...
    uart2_flush();
    UART2_SendString(WS_REQUEST_STR);

    if (!uart2_read_json_response(rx_json, sizeof(rx_json))) {
        return;
    }
